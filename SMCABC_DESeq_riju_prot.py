...
def test_parset(parset, obs_data):
    ...
    score[4:] -= 0.3            # 0.3 chosen by eyeballing what a 0.2 diff is on the graphs
...
#        print('hit!')
    ...
    while proceed:
        ...
        pool = mp.Pool(processes = pool_number)
        for i in range(hits_needed):
            pool.apply_async(get_successfulSMC_parset, args=(threshold, all_good_params[l], weig[l], covar, obs_data), callback=add_success)
        pool.close()
        pool.join()
