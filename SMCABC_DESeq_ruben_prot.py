...
# ODE functions
paramnames = ['r_s', 'beta_s', 'K_NI', 'K_OI', 'K_NP', 'K_OP', 'K_IO', 'K_NO', 'K_GO', 'K_IN', 'K_PN', 'K_ON', 'K_GN', 'K_LN', 'K_LP', 'K_LI','K_LO']
...
def test_parset(parset, obs_data):
    ...
    score = np.sqrt(np.sum((go-obs_data)**2, axis=0))
    score[4:] -= 0.3            # 0.3 chosen by eyeballing what a 0.3 diff is on the graphs
    if np.max(score) <cutoff1:
        return False
    else:
        return True
...
#        print('hit!')
        ...
        good_scores.append(np.max(score[0:4])) # only involve the mRNA in the scoring
...
def test_SMCparset(parset, thresh, obs_data):
    ...
    score = np.sqrt(np.sum((go-observed)**2, axis=0))
    score[4:] -= 0.3
    if np.max(score) <thresh:
        return False
    else:
        return True
...
def SMCABC_run(good_params, good_runs, obs_data, good_scores):
    ...
    all_thresh = [cutoff1, max_good_scores[0][int(prop*hits_needed)]]
...
#        print('hit!')
        ...
        score = np.sqrt(np.sum((go-observed)**2, axis=0))
        final_good_scores.append(np.mean(score[0:4]))
        max_good_scores[l+1].append(np.max(score[0:4]))
    ...
    while proceed:
        ...
        max_good_scores[l+1].sort()

        threshold = max_good_scores[l+1][int(prop*hits_needed)]
        if (all_thresh[-1]-threshold) < 0.1: # to prevent getting stuck in local minima
            print('Thresholds converging, force reduceby 0.1')
            threshold = all_thresh[-1]-0.1

        if threshold < target_thresh:
            threshold = target_thresh

        if threshold < target_thresh+0.1:
...
def plot_trajectories(run_output, thresh):
    ...
    for i in range(len(run)):
        ...
        ax.plot(timepts, run[i][:, 4], ":", color="peachpuff")
        ax.plot(timepts, run[i][:, 5], ":", color="palegreen")
        ax.plot(timepts, run[i][:, 6], ":", color="lightcoral")
        ax.plot(timepts, run[i][:, 7], ":", color="cornflowerblue")
...
def get_posteriors(observed, params, good_scores, thresh):
    ...
    ax.plot(timepts, simulateposterior[:, 4], ":", color="orange")
    ax.plot(timepts, simulateposterior[:, 5], ":", color="green")
    ax.plot(timepts, simulateposterior[:, 6], ":", color="red")
    ax.plot(timepts, simulateposterior[:, 7], ":", color="cornflowerblue")
...
experimental data  points (day 4):  [	0.45,	 0.15,	 0.5,	0.95, 	n/a, 	n/a,	n/a,	n/a]

simulated data points (day 4):		[	 0.2,	  0.1, 	 0.45, 	 1.0,	0.4, 	0.4,	0.6,	0.8]  (- then **2)
																										
-----------------------------------------------------------------------------------------------------------
squared difference (day 4):			[ 0.0625, 0.00255, 0.00255,	0.00255, n/a,	n/a,	n/a,	n/a	]
-----------------------------------------------------------------------------------------------------------

final score = np.sqrt(np.sum((difference across all days)))

							e.g.	[ 	0.2, 	0.3, 	0.35,	0.25,	 n/a,	n/a,	n/a,	n/a	]
...
experimental data  points (day 4):  [	0.45,	 0.15,	 0.5,	0.95, 	0.45,	 0.15,	 0.5,	0.95]

simulated data points (day 4):		[	 0.2,	  0.1, 	 0.45, 	 1.0,	0.4, 	0.4,	0.6,	0.8]  (- then **2)
																										
-----------------------------------------------------------------------------------------------------------
squared difference (day 4):			[0.0625, 0.00255, 0.00255, 0.00255, 0.0625, 0.00255, 0.00255,0.00255]
-----------------------------------------------------------------------------------------------------------

final score = np.sqrt(np.sum((difference across all days)))

							e.g.	[ 	0.2, 	0.3, 	0.35,	0.25,	 0.2, 	0.3, 	0.35,	0.25]
							
									[ 	0.0, 	0.0, 	 0.0,	 0.0,	 0.3, 	0.3,  	 0.3,	 0.3]  (-)
-----------------------------------------------------------------------------------------------------------
									[ 	0.2, 	0.3, 	0.35,	0.25,	 -0.1, 	 0.0, 	0.15,	-0.05]
-----------------------------------------------------------------------------------------------------------
